...
//!   nothing in the file says so: read as radians, `<angleoffset>180</angleoffset>` turns a fin
//!   more than fourteen times round. 86 of the angles in the reference corpus are larger than 2π.
//!
...
pub const ATTACHED_TAGS: [&str; 15] = [
    "innertube",
    "tubecoupler",
    "engineblock",
    "centeringring",
...
fn one(
    ...
    // none to give. The layout would refuse the whole design over it, so the part goes instead.
    if !auto.is_empty() && !matches!(parent, Part::BodyTube(_) | Part::InnerTube(_)) {
        values.warn_at(
            WarningKind::Skipped,
            format!(
...
/// reference corpus are written this way, two of them in OpenRocket's own example designs.
...
fn fin_set(element: &Element, values: &mut Values<'_>) -> Option<Part> {
    ...
    if count == 0 {
        values.warn_at(
            WarningKind::Skipped,
            "a fin set of no fins; it was left out",
        );
        return None;
    }
...
fn tab(values: &mut Values<'_>, root_chord_m: f64) -> Option<FinTab> {
    ...
    let offset_m = match relative_to.as_str() {
        "front" => offset_m,
        "center" => 0.5 * (root_chord_m - length_m) + offset_m,
        "end" => root_chord_m - length_m + offset_m,
        other => {
            values.warn_at(
                WarningKind::Unusual,
                format!(
...
fn tube_fins(values: &mut Values<'_>) -> Option<Part> {
    ...
    if count == 0 {
        values.warn_at(
            WarningKind::Skipped,
            "a tube fin set of no tubes; it was left out",
        );
        return None;
    }
...
fn launch_lug(values: &mut Values<'_>) -> Option<Part> {
    ...
    let (stated_m, outer_radius_m) =
        stated_radius(values, &["radius"], AutoDimension::OuterRadius, &mut auto);
    let thickness_m = tube_wall(values, stated_m)?;
    let (count, spacing_m) = row(values);
...
/// How many of an instanced part there are, and how far apart they sit along the body.
fn row(values: &mut Values<'_>) -> (u32, f64) {
    (
        values.count(&INSTANCE_COUNT).unwrap_or(1).max(1),
...
fn position(values: &mut Values<'_>) -> Position {
    ...
    let from = element
        ...
        .unwrap_or("top")
        .to_owned();
    let aft_offset_m = values.number(&AXIAL_OFFSET).unwrap_or_default();
    match from.as_str() {
...
/// metres, so reading one as radians is an easy mistake that turns a fin 90° away into one turned
/// more than fourteen times round. The corpus settles it: of the angles written in it, 86 are
/// larger than 2π — more than a whole turn, if they were radians — and the values themselves are
/// 180, 90, 45, 30 and 120, with float dust (`119.99999999999999`) from a conversion that went
