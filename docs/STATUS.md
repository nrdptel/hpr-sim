...
- **Current milestone:** M1.8e — all done bar M1.8e16, `[blocked]` on #108; the work is **M3.1b**,
  the `.ork` component tree, now M3.1a has shipped the container.
- **Order:** M3.1b, M3.1c, M3.1d, then M2.2, then M1.9; M1.8e16 (the handover past 24°) waits on #108
- **Run:** M0.1-M0.4, M1.1-M1.7, M2.1, M1.8a to e19 bar e16, M3.1a;
...
- **Last updated:** 2026-09-20 (M3.1a shipped, ADR-051; M3.1b is next)

## Handoff (overwrite each session)

- **Page rules** (ADR-016 to ADR-020): relative links between pages, GitHub URLs for the rest
  (including from rustdoc), labels as links to their rows, none in headings, Unicode equations; new
  pages in `SUMMARY.md`; a new library a row in `docs/api.md`. *Accuracy*'s numbers live in a file
  the item links; each aero fixture has a `the_guide_quotes_the_fixture`. A milestone's
  `decisions-and-roadmap.md` row must say `done` (or `blocked`) to match; an id carries one
  increment level. `STATUS.md` holds 150 lines and `ROADMAP.md` 1000: trim an old entry to add one.
- **Validation (M2.1, ADR-021 to ADR-026):** CI checks the report on three OSes; predicted mode's 3%
  ...
  `arcas-robin-gap.json`'s last digits whatever you changed — `git checkout` it.
- **M1.8a to e19** (ADR-027 to ADR-050). What the measurements came to is in the guide's aero page
  and in *Known issues* below; what a session needs to work on it: `cargo xtask aero` writes the
  aero fixtures (five of ADR-030's PDFs come from NTRS with a 436-byte header, scratch
  `refs/scratch/m18*/`); `SupersonicBody` tabulates every 0.05 Mach from max(1.2, its start),
  joined over 0.3; `BEFORE_M1_8E6` keeps the old rules and `CONE_SLOPES` runs to 30°; a
  mesh-following answer is marked by the pressure **crossing** its tangent cone's, not `η < 0`;
  the near-flat flare's two edges are solved by `flare_reduction_turns_rad`, and what is left of
  it is the crossing's pole in the loading, `crossing_loading_gap_per_rad` (#108). A **step** in
  radius still takes the whole body off the method (#87, #120, #121), and a boattail's or
  cylinder's reduced element still refuses (#123), which is what keeps ADR-038's Newtonian-start
  rows in `blunt-tips.json`.
- **Debrief, folded in** (ADR-046): `hpr-flightdata` is off `hpr-sim` and must stay off it
  ...
  `hpr-forensics`. Notes: `debrief-{log-formats,flight-readings,porting-boundary}.md`. **Port from
  its `lib/`, never from its `COMPETITION.md`** (GPL-3 Java); its 12 public fixtures may be used.
...
- ADR-049: a step in radius keeps the model it has (no source gives a step's normal force faster
  than sound); its size is published instead, and stopping the march at the step was built and
  rejected. #87 is narrowed to the step, #120 and #121 carry the rest.
- ADR-048: model 2's drawing is closed on the base rather than on its printed lengths, and the
  spread published; a blunt nose may span more than one curved segment but a cap may not reach a
  cylinder; the Mach 1.50 refusal stands; the three separated rows are a flow hpr doesn't model.
...
- Aero (M1.5a) is small-angle only; the Recruiter's six fins miss the printed slope by +3.42%
  ...
  0.19 on a short shoulder (#108); a step in radius takes the whole body off the method past
  2.7e-11 m tube to tube or 1.3e-13 m up at a boattail — −8.65% to −11.34% (#87).
- `.ork` (M3.1a) reads the container and the document only: no rocket is built from it yet, so
  nothing in the reader has been checked against what OpenRocket means by a tag.
...
- Flight: no tip-off, turbulence or thrust misalignment; small-angle aero at every `α`. Recovery: no
  ...
  Kidwell's); tumble +19% (M1.7b). Results are not bit-identical across the three OSes: the report
