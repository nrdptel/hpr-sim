...
it can change, and a simpler one is planned ([M4.1][roadmap], a builder API).
...
| [`hpr_motor`](api/hpr_motor/index.html) | Solid motors: thrust curves, mass and inertia through the burn, `.eng` and `.rse` files, and the 32 bundled curves | Hybrid and liquid motors, which are out of scope | [Solid motors](physics/motor.md), [`.eng` files](format/eng.md), [`.rse` files](format/rse.md) |
...
| [`hpr_aero`](api/hpr_aero/index.html) | Aerodynamics: normal force, centre of pressure, drag and drag tables | Mach 1 and above ([M1.8][roadmap]) | [Aerodynamics](physics/aero.md) |
| [`hpr_sim`](api/hpr_sim/index.html) | The flight: the launch rail, the equations of motion, time integration, events and recovery | Staging under power, clusters and air starts ([M1.9][roadmap]) | [How a flight is simulated](how-a-flight-is-simulated.md), [Rigid-body flight](physics/flight.md), [Time integration](physics/integration.md), [Recovery](physics/recovery.md) |
| [`hpr_validate`](api/hpr_validate/index.html) | The validation harness: cases, reference data, metrics and reports | Whole flights against RocketPy ([M2.1b2][roadmap]) | [Accuracy](accuracy.md), [Checking a claim](checking-a-claim.md) |
...
| [`hpr_io`](api/hpr_io/index.html) | Import and export of OpenRocket, RockSim and RASAero designs, export to RocketPy, and OpenRocket's parts database | [M3.1][roadmap] to [M3.6][roadmap], [M5.5][roadmap] |
...
| [`hpr_analysis`](api/hpr_analysis/index.html) | Monte Carlo dispersion, sensitivity analysis, optimization, and competition challenges such as a target apogee | [M6.1][roadmap] to [M6.3][roadmap] |
