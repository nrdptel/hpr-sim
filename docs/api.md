...
| [`hpr_sim`](api/hpr_sim/index.html) | The flight: the launch rail, the equations of motion, time integration, events and recovery | Staging and [air starts](glossary.md#air-start) fly, checked by tests but not yet against another simulator ([M1.9c](decisions-and-roadmap.md#m1-9c)). A [cluster](glossary.md#cluster) flies, one mount of several tubes or one mount per motor, and so does a motor out, checked by tests against a hand calculation; a `.ork` file's clusters fly from [M1.9c](decisions-and-roadmap.md#m1-9c) | [How a flight is simulated](how-a-flight-is-simulated.md), [Rigid-body flight](physics/flight.md), [Time integration](physics/integration.md), [Recovery](physics/recovery.md), [Staging](physics/staging.md) |
...
| [`hpr_io`](api/hpr_io/index.html) | Import and export of OpenRocket, RockSim and RASAero designs, export to RocketPy, and OpenRocket's parts database. A `.ork` file's container, design document and components are read into a design ([the format page](format/ork.md)); its motors and recovery settings are not, so an imported design cannot yet be flown | [M3.1a](decisions-and-roadmap.md#m3-1a) done; [M3.1b](decisions-and-roadmap.md#m3-1b) to [M3.6](decisions-and-roadmap.md#m3-6), [M5.5](decisions-and-roadmap.md#m5-5) next |
