...
| [Aerodynamics](physics/aero.md#the-body-alone-against-the-15-target) | the Arcas Robin's body alone, fins off, against the 15% target the milestone set before the work began, 11 readings at Mach 1.5 to 4.63 ([fixture][body-gap-fixture]) | not met on six rows: the short model +37.7% at Mach 1.5, +25.9% at 1.8, +16.9% at 2.3 and 2.96, and the long +19.4% at 1.8 and +15.5% at 2.3; from Mach 3 both are within 5%. At `α → 0` hpr is within 1.5 standard errors of the measurement on every row outside, so the resolvable gap is in the curvature body lift adds: 1.3 to 2.8 times the measured below Mach 3, and 0.89 to 1.33 times it above |
