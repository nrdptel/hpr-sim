...
1. **M2.2b2 splits.** b2 takes the fins, the rail button and the roll inertia. A new b3 takes
   ...
   or b3's.
2. **Measured on probes.** `validation/oracles/openrocket/conventions.py` gains 31 probes, each a
   tube and one part: fin sets of every outline, section, a tab, fillets and a cant; rectangles of
   two chords, two spans and on two tubes; every attached and packed part; rail buttons, one or a
   row of two, from each end. `hpr_validate::openrocket::tests` holds hpr to each or pins the gap.
   The tube alone is OpenRocket's, so each probe measures its one part.
3. **Roll inertia is the fins, and it is explained.** On the probes every part but a fin set is
   OpenRocket's in roll inertia to 1e-15. A set of two or more fins is not. OpenRocket gives it the
   set's mass times `R² + R hₑ + hₑ²/3`, with `hₑ² = A h / c_r`: the mean of `r²` over a flat strip
   from the body at radius `R` out to `R + hₑ`, where `A` is one fin's area, `h` its span and
   `c_r` its root chord. This is **inferred from OpenRocket's output** (its public
   `getRotationalUnitInertia`), not taken from its source. It holds to 1e-15 on every fin probe
   but two: an ellipse (OpenRocket's is a 30-sided polygon) and a cant (−2.66e-5 of the probe's). A tab's mass
   takes the planform's value, and neither the section nor the thickness enters. hpr keeps its own,
   the exact integral of `r²` over the fin, checked against hand calculation: **a departure**. The
   two agree on a rectangle without a tab, bar the thickness term OpenRocket leaves out. They part
   on a tapered fin (hpr 2.4% higher on the probe's trapezoid) and more on a tab, whose mass
   OpenRocket puts 72.7 mm from the axis where the tab lies 40–50 mm out (hpr 5.1% lower).
...
4. **Fin sections: hpr's own, pinned.** OpenRocket weighs a fin set as outline × thickness ×
   ...
   So hpr keeps its sections, a departure; the ratios are pinned. The elliptical fin stays the
   exact ellipse too (OpenRocket's polygon is 0.18% lighter).
5. **A rail button's place is OpenRocket's (#151).** OpenRocket gives a button no length. It
   ...
   top, middle and bottom, one button and two. After and absolute are read the same way,
   unprobed.
6. **Findings left for b3, each pinned.** A parachute that writes no packed size is packed 25 mm
   long and 12.5 mm in radius by OpenRocket; hpr reads no radius, with a warning. A mass override
   on a packed part that weighs nothing is spread over the packing by OpenRocket and is a point
   mass in hpr. Fillets' mass (0.81% of the probe's structure at 5 mm) is still left out. Untraced
   and small: a cant's mass (−4.19e-5 on the probe's structure), fins' pitch inertia (up to 0.11% of the
   probe's where the fins weigh the same; OpenRocket's pitch rule for fins is not measured), a lug's pitch (3.1e-4), and a rail button's
   inertias (up to 5.0e-4).

**Consequences.** On 2026-09-21 (`cargo xtask ork`, OpenRocket's record unchanged):
- **Roll inertia with OpenRocket's fin rule in hpr's place:** median 0.141% (hpr's own: 2.112%);
  within 0.1% on 33 of 74 (was 11) and within 1% on 53 (was 29). Five of Loft's six public demos
  come within 5e-6. The sixth, `demo-boattail.ork`, is 0.232% apart; with OpenRocket's polygon
  ellipse it is 1.5e-6 (a test).
- **The 21 files still outside 1%** (17 by content) each have a cause the survey names, or it
  fails. By content: a mass override covering the parts inside (ADR-061's departure) 7, a reduced
  design 6, a fin set OpenRocket weighs otherwise 8, a packed part hpr weighs as a point mass 5.
  Some files have two.
- **Mass and centre of mass** unchanged: 61 and 62 of 74 within 1%. The rail button moved no file
  across a threshold.
- **hpr's own numbers do not change** except a `.ork` rail button's station.
