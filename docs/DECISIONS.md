...
3. **A powered separation keeps the nose body flying.** When the separation fires and body 0 still
   ...
   the design cut after the boundary: the sustainer keeps the nose, so it needs no headless model.
   Only a body without the nose would, and that body is the booster, which descends as a point mass
   under its own device as in ADR-014. The booster's motors must be spent: a separation while one
   burns or is still to light is refused, before the flight when the time is known and in flight
   otherwise.
...
6. **Triggers and events.** `Trigger::Burnout { motor, delay_s }` fires a delay after a motor's
   ...
   recorded when every motor lit, or due at a known time, has burned out, and again if one lights
   afterwards, as a sustainer lit by its separation does.
7. **Refused rather than guessed.** A drag or normal-force table is the whole stack's, so a
   ...
   body 0, which the point-mass descent can't fly under thrust.
8. **Found on the way.** A body whose device opened on the stack before the separation was refused
   ...
   and asserts that the booster is the body refused.
...
- M1.9a's bar is met by tests in `hpr_sim::staging` and `hpr_design::config`. They cover the
  ...
  height separation firing in flight with both bodies landing, linear momentum, and the refusals.
- The staged flight is not validated. M1.9c compares it with OpenRocket; until then the staging
  page says so.
- Not modelled: an ejection or separation impulse, the flow between separating bodies, a motor's
  ignition transient beyond its curve, and the booster's attitude (ADR-014).
