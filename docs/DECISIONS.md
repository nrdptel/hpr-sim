...
2. **The committed record covers the bundled curves only.** They are public-domain files already in
   ...
   not the curves.
3. **What is held, and what is recorded.** A test in `hpr-motor` holds hpr's total impulse to the
   milestone's 0.1%, and its peak thrust to rounding, on all 32 curves, tying the record to each
   file by SHA-256. Measured: both differences are **exactly zero** at `f64`. Both codes integrate
   the listed points trapezoidally, and every bundled curve's first point is at ignition, so
   nothing rounds differently.
4. **Two differences are written down rather than held, and one apparent one is not real.**
   - **Burn time.** OpenRocket's `getBurnTime` is its own window; hpr's is [NFPA
     1125](glossary.md#nfpa-1125)'s 5% rule as ThrustCurve.org applies it. Over the 32 curves hpr's
     burn time is up to **12.37%** from OpenRocket's, median **3.19%**, and its average thrust —
     impulse over that window — up to **0.31%**, median **0.10%**. The test prints all four, so the
     sizes come from the suite and not from this page. hpr keeps the published rule it cites.
...
**Consequences.** The 0.1% half of M2.2c is met, and met with room to spare: hpr's integration of a
curve file is OpenRocket's, bit for bit, on every curve it can publish. What remains for M2.2c2 is
supply, not arithmetic — the 193 motor references in the reference library that resolve to no curve,
and the configurations they hold back. Because the two burn-time definitions differ by up to 12%, a
later milestone that compares a *flight* against OpenRocket must not compare burn times across the
two codes without saying which definition each uses; the numbers above are that warning, with their
size. The record is regenerated by running the script, never edited; it carries the jar's SHA-256,
