...
- **Friction.** Fully turbulent (Niskanen p. 43; RASAero II defaults to laminar flow tripping at
  `R = 5e5`, a small difference at rocket Reynolds numbers), `R` on the rocket's length (nose tip to
...
- **Pressure drag.** Noses and shoulders `0.8 sin² φ`, `φ = atan(dr/dx)` at the aft joint, on the
  ...
  hpr reads both as the decrease in area (Calisto's boattail: 0.052, against 0.046 the other way). Steps in radius are zero-length shoulders (`0.8 ΔA`) or boattails (base
  drag on `ΔA`); a body without a nose cone gets `0.8 A` on its face.
- **Above Mach 0.8.** Niskanen's subsonic methods stop at Mach 0.8 (Table 3.1). Eq. 3.87 then
  raises nose and shoulder pressure drag toward appendix B's transonic values (closed forms for
  cones and ogives, Stoney's data for other shapes); all of it arrives with M1.8. The buildup
  accepts Mach numbers up to 1 so M1.6 can fly, and sets `Drag::beyond_subsonic_methods` above 0.8,
  where `C_D0` is low (a 3:1 cone's pressure drag would gain about 0.05 by Mach 0.9).
- **Base drag.** Eq. 3.94 on the last body component's aft area less the thrusting motors'
  cross-section (`DragConditions::thrusting` with the burning motors' case area; Niskanen p. 50),
  down to zero.
...
- **The comparison with RocketPy's curves.** Every RocketPy example whose drag curve is labelled
  ...
  - Juno III's, Cavour's and Valetudo's are labelled RASAero but are 3-decimal tables with no input
    ...
    power-on tables, which are compared too. Cavour's table repeats seven Mach numbers below 0.1
    with values 0.001 apart; the comparison keeps the first of each.
- **Inputs for the comparison: one declared rule, placeholders where nothing is known.** The
  ...
  section the rocket's team published is used (Juno III: "aletas com perfil de aerofólio truncado",
  a rounded leading edge and a blunt trailing edge, taken as rounded); otherwise the M1.4b
  placeholder, square 3 mm (Calisto's 2018 fins, Cavour, Valetudo). A lift-curve airfoil says
...
- **Result, and the gaps it leaves.**
  ...
  cases outside 10%:
  - **Power-on base relief differs from RASAero's.** At Mach 0.3, subtracting the motor's whole
    area removes 42% of Cavour's base drag (0.055) and 29% of Valetudo's (0.038); their tables drop
    0.0001 and 0.004. hpr follows Niskanen; M2.1's flights will show which is closer.
  ...
  - **What the passing cases show.** Unrecorded inputs move each by 20% or more, and three of four
    rockets pass only with the fin sections chosen above. The check places hpr near RASAero's
...
- M2.1's same-drag mode uses `DragTable`. It must decide whether RocketPy's drag scales with angle
  of attack as hpr's does, and its flights test the power-on base relief.
