...
- **Body lift is Jorgensen's `η C_dn (A_p/A_r) sin² α`** (TR R-474 eq. 2.12), at every Mach
  ...
  so for fineness `f` hpr takes `η = η₄(f) + [1 − η₄(f)] (η₆ − 0.69)/0.31`, a judgement that keeps
  Fig. 4 at low `M_n`, gives Fig. 6 back near `f` = 10.3 and stays below 1. Both curves are sampled
...
- **A supersonic boattail takes Washington and Pettis's measured increment** (MICOM RD-TM-68-5,
  ...
  anything else is an extrapolation, stated, and past the curve's end the last value is held. A
...
- Like for like, the fitted-nose body reads +3.4% to +41.0% (`arcas-robin-crossflow.json`): each
  ...
  +7.3%); Mach 1.5 to 2.96 still read 16% to 41% high, mostly the slope at `α → 0`.
...
- **The moment, checked** (a partial model swap can match a slope with its lift in the wrong place):
  ...
  −0.18 (±0.5 from the readings). Where the crossflow is supersonic (`M_n` ≥ 0.95) hpr
  now reads 4% to 17% high, where Galejs's constant read 7% to 22% low.
