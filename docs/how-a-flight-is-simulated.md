...
in height, speed and time when both codes fly the same drag, but not in where they go in wind;
with hpr's own drag, against RocketPy flying the drag its examples ship, heights differ by
−0.527% to +10.232% ([Accuracy](accuracy.md#whole-flights-with-each-codes-own-drag)); and
