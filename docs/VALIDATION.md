...
| RocketPy `Rocket` with a motor | total mass, centre of mass and inertia vs time for eight example rockets | MIT | `validation/oracles/rocketpy/rocket_mass.py` → `validation/fixtures/design/rocketpy-rocket-mass.json`: each example's own inputs, with the bundled public-domain curve nearest in impulse in place of its thrust file (ADR-007). hpr's designs (`validation/designs/`) agree within 1.4e-5 in mass, 3.9e-6 of the length in centre and 2.6e-5 in inertia; dry values to 2e-16 (M1.4b, `docs/physics/design.md`) |
