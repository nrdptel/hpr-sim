...
- **Always regenerated.** `validation/reports/latest.md` (plus JSON) is regenerated by
  `cargo xtask validate`. It lists every case, error, tolerance and verdict. The accuracy census
  (`validation/reports/census.md`, [the census](accuracy.md#the-census)) counts every number the
  committed reports compare.
