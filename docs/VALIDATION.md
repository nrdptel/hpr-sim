...
| source | what | license | notes |
|---|---|---|---|
| C. Kidwell, *Streamer Duration Optimization*, NAR R&D, NARAM-43 (2001) | free-drop descent rates and per-streamer masses for sixteen 4 in × 40 in streamers over 20.1 m | no terms stated: cited, never redistributed | the measurement both streamer models are checked against, recomputed in `hpr_sim::recovery::tests::streamer_models_against_kidwells_drop_tests`. His unpleated crêpe streamer descends at 2.80 m/s, a `C_D` of 0.161 on the planform area: Carruthers and Filippone's correlation gives +12%, the OpenRocket technical documentation's appendix C +110%. His pleated streamers descend slower than either model, and hpr models no pleats |
| J. Carruthers and A. Filippone, *J. Aircraft* 42(4), 2005 | wind-tunnel drag of cotton streamers, `AR` 3.3 to 30, 6 to 18.9 m/s | paywalled; the authors' post-print states no terms: cited, never redistributed | hpr's default streamer model (eqs. 1 and 2), pinned by `streamer_models_reproduce_their_printed_equations` |
| OpenRocket technical documentation v13.05 | appendix C's streamer correlation and §3.5's tumbling model with its fin efficiency table | CC BY-SA | both transcribed with their printed pages and pinned by tests; §3.5's constants are fitted to 6.8 to 160 g models at 5 to 6.6 m/s (3 to 14%), so a high-power body tumbling is an extrapolation (`docs/physics/recovery.md`) |
