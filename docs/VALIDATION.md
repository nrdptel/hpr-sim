...
| OpenRocket 24.12 jar | second oracle (run only, never read its source) | GPL-3.0 | `https://github.com/openrocket/openrocket/releases/download/release-24.12/OpenRocket-24.12.jar` | Needs Java 17 (see [issue 94](https://github.com/nrdptel/hpr-sim/issues/94): the jar refuses 21, though our check accepts it). How M2.2 drives it is open: JPype directly, or the jar as a subprocess. The GPL-2.0 **orhelper** wrapper was dropped from the environment rather than imported. 16 example `.ork` files are in the jar under `datafiles/examples/` (use them locally, don't commit them) |
