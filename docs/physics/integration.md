...
- **Step control** ([HNW] II, §IV.2; [D5] defaults). The PI controller
  ...
  A non-finite error (a NaN from the system at a too-large step) counts as a rejection that
  shrinks `h` five times.
...
- `Integrator::advance(system, t_stop, events, observer)` never steps past `t_stop`. Like `DOPRI5`,
  it stretches the last step up to 1% to land on it exactly, and `time_s()` equals `t_stop` bit for
  bit.
...
- After each accepted step, `advance` evaluates every `g` at the step's end. For each sign change it
  ...
  `EVENT_TIME_RESOLUTION_S = 1e-12 s` (plus `4ε|t|`). The earliest zero wins, and the lowest index
  breaks a tie.
- **Where it stops.** The integrator stops at the end of Brent's final bracket on the far side of
  the zero, with the dense output's state there. So `g` is already past zero (or exactly zero) when
  the next call starts, and the event can't be reported twice. `Step::truncated` gives the
  observer the shortened step.
...
- There is no stiffness detection (`DOPRI5` has one). A stiff flight phase shows up as
  `StepTooSmall` or the step limit instead.
- Integration runs forward only. `t_stop` may be infinite, to run until an event.
...
- **Tableau.** Rows sum to `c`. The fifth-order weights satisfy all 17 order conditions to
  ...
  fifth-order condition by more than 1e-4, which catches a typo in any coefficient.
- **Step-halving convergence** on the nonlinear, non-autonomous `y' = −2ty²`, `y = 1/(1 + t²)` on
  `[0, 2]`:
  - Fixed-step Dormand–Prince, 10 to 320 steps: orders 5.88, 5.53, 5.31, 5.16, 5.09, falling toward
    5. The dense output at `θ` = ¼, ½, ¾ gives 4.81, 4.97, 4.99, 4.99, 5.00.
...
- **Tolerance proportionality.** Over `[0, 10]` with `rtol = atol` from 1e-4 to 1e-9, the error
  falls from 2.7e-5 to 2.2e-10, about 0.25 of the tolerance throughout.
- **Apogee under tolerance halving (L21).** A vertical flight with quadratic drag, `v' = −g − kv|v|`,
  ...
  3.4e-4 m to 8.3e-8 m. The fall is not monotone step by step: `v|v|` has a kink at apogee.
...
- **Brent.** Checked on a smooth root to 1e-14, a triple root, a step discontinuity (under 60
  ...
  NaN.
- **Failures.** A failing derivative, a blow-up (`y' = y²` stops near `t = 1`), the step limit, bad
  settings, a backward stop and zero weights are reported as errors, never as a quiet stop.
