...
Before M2.2b1 it was 57 and 58 of 74 (median mass 0.020%).
...
**A worked example, now settled.** In M2.2a the OpenRocket jar's *Two stage high power rocket* was
...
and since M2.2b1 so does hpr, so the file is within 1% in both mass and centre of mass. Two causes
M2.2a counted, this shoulder of no wall (6 files by content) and a part written with no material
(1), are gone the same way.
...
parts inside it both override their mass, which wins? OpenRocket, which writes these files, has an
answer to each, and its answer is what the file means to the person who wrote it. So hpr asks it:
`validation/oracles/openrocket/conventions.py` writes 27 small probe designs, each asking one
question, runs OpenRocket 24.12 on them and records its answers. The test module
`hpr_validate::openrocket::tests` reads the same designs with hpr and holds hpr to them. Where hpr
keeps a rule of its own, the test pins how far apart the two are. This was
[M2.2b1](../decisions-and-roadmap.md#m2-2b1); [ADR-061][adr-061] records the decisions.

**Read as OpenRocket reads it.** On every one of these probes hpr's mass and centre of mass are
OpenRocket's, part by part as well as whole:

| the file says | what it weighs (OpenRocket 24.12, and now hpr) |
|---|---|
| a nose cone, transition or body tube with a wall of 0 | nothing: the part keeps its shape (so its drag) but has no wall; a solid part is written `filled` |
| a shoulder with a wall of 0, or none written | nothing, capped or not, on a hollow nose or a filled one |
| a filled nose cone with a walled shoulder | the solid cone plus the shoulder's own wall |
| a nose cone, transition or body tube with no thickness written | a 2 mm wall, whatever its radius (measured at 50 mm and at 30 mm) |
| a solid part with no material | cardboard, 680 kg/m³ |
...
weighs 0.091726 kg again, in both programs. Before M2.2b1 hpr read that shoulder as solid, and
would have added `π × 0.048² × 0.1 × 1000` = 0.724 kg.

**Which override wins.** An override is a number the designer typed in place of what the parts add
up to, usually after weighing the real thing. The probes find hpr and OpenRocket agree on which one
wins, and on where a centre is measured from:
...
**Two rules stay hpr's own.** Each is a *departure*: hpr knowingly differs from OpenRocket, and a
test pins by how much.

| when | hpr | OpenRocket | apart on the probe |
|---|---|---|---|
| a mass override covers the parts inside and states no centre | keeps the centre the parts lay out | puts it at the overriding part's own, leaving the parts inside out | 3.7 mm, or 19.7 mm if the part inside has an override of its own |
| a mass override covers more than one part | scales the inertia of everything it covers by the override's ratio | scales only the overriding part's own inertia; a stage, having none of its own, scales nothing | roll inertia 6.9% to 37% lower in hpr under a tube's; 2.5 to 5.0 times OpenRocket's under a stage's |
...
once, takes the mass's, and warns. On the probe that is 4.7 mm.
...
**What it leaves out.** Each probe asks about one kind of part at one size, so a reading is
measured, not proven for every case. OpenRocket's defaults are read with its preferences as a fresh
install sets them. A missing thickness on an inner tube is not probed (hpr reads it as no wall; no
file in the reference library has one).
