...
- **How well it is validated:**
  ...
  - *The body faster than sound*, by the method a flight blends in over 0.3 in Mach from Mach 1.2
    ...
    - *Like for like*, fitted at the tunnel's angles with the [body lift](../glossary.md#body-lift)
      a flight adds, the body with its boattail reads 14.9% to 73.2% high: hpr's body lift is
      larger than the tunnel's there.
...
- **What it leaves out:** large angles and [stall](../glossary.md#stall), though a flight uses
  ...
  steps yet; [M1.8e6](../decisions-and-roadmap.md#m1-8e6) (resizing body lift faster than sound,
  and blunt tips) may cover blunt tips. There are no damping coefficients for pitch and
...
lift. A flight adds body lift (`K` = 1.1) on top, and at the tunnel's angles that reads too large
(see [Checking the shock-expansion method](#checking-the-shock-expansion-method)).
...
- With that curvature fitted out, hpr's slope at `α → 0` lies between 0.37 below and 0.87 above
  the tunnel's. The tunnel's points were read off plots to ±0.01 in `C_N`, so its slope there is
  uncertain by ±0.2 to ±0.4 per radian, and the readings can't say which is right.
- The lip, which the method can't take, adds +0.178 by slender-body theory. Below Mach 3, where
  the tangent cones' slopes are held at TN 3527's Fig. 2 Mach 3 curve, Sims's tables ([S64]
  Table 2, p. 20) move the nose and cylinder's share by −0.033 to +0.057 at most. hpr's reading
  of [issue #81](https://github.com/nrdptel/hpr-sim/issues/81) (how it continues the method
  where TN 3527's relaxation condition fails) changes nothing on this body.
