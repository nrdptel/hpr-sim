...
- **Nozzle gyration tensor.** `S_k = (r_e²/4) diag(1, 1, 2) + |n_k|² 1 − n_k n_kᵀ`, integrated here
  ...
  - RocketPy 1.13.0's code (`rocket.py:984-985`, `evaluate_nozzle_gyration_tensor`) uses
    `0.25·n²` for the transverse distance term instead of `n²`. M2.1 must account for the
    difference when it compares with RocketPy's generalized flight.
...
- **Time derivatives of the mass properties.**
  ...
  - After burnout (an interval starting past every burnout) the rates are zero.
...
- **Damping.** The rotation's contribution to each local flow is the only pitch and yaw damping.
  ...
  `|ω × p_i|`.
- **Limits.**
  - The models are small-angle (fin normal force linear in `α`, no stall). They are used as they
    are at every angle, which overstates the forces at large `α`. Large `α` occurs near apogee,
    where the dynamic pressure is small. `Sample::angle_of_attack_rad` shows where it happens.
...
- **Rail.**
  ...
  - If the speed along the rail falls to zero, the rocket returns to the pad phase, at rest.
...
- **Rail geometry.**
  ...
  - Friction is Coulomb friction with a user coefficient. No source gives a default, so the
    default is zero.
...
- **Accuracy against cost.** Measured on Valetudo (K400C), with a 5 m/s wind from the west,
  compared with the apogee at 1e-11 (873.933824 m):

  | `rtol = atol` | apogee error | flight time (release) |
  |---|---|---|
  | 1e-4 | 0.18 m | 0.38 ms |
  | 1e-6 | 3.8 mm | 0.60 ms |
  | 1e-8 | 2.7e-5 m | 1.12 ms |
  | 1e-10 | < 1e-6 m | 2.82 ms |
...
- **L20, weathercocking.** In a 5 m/s wind from the west, Valetudo's axis is 0.12 west of vertical
  at burnout. Its apogee is 96 m upwind, against 1.0 m (Earth rotation) in calm air.
...
- **Refusals.** The synthetic 54 mm rocket on an I175 passes Mach 1 and stops with
  `SimError::Aero(Mach)`.
- **Cost.** 1.09 ms per Valetudo flight to the ground (`docs/perf.md`).
