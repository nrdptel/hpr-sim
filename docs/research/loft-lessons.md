...
`refs/fusionspace-loft`. Private-corpus facts appear only as counts.

**Rules.** Each `L` row names its milestones (the first owns the tests) and the tests to write.
Each named milestone's `ROADMAP.md` entry lists the id, and a checked-off milestone's tests must
exist; `xtask/src/docs.rs` enforces both. Renaming a test or moving a lesson later is fine (note
it in `STATUS.md`); dropping one needs an ADR. `P` rows name the guard against a process mistake.
Loft's claims are leads: every test re-derives its numbers from a primary source or an oracle.

## What Loft was

- **Dynamics and environment:** 3-DOF point mass, thrust along the velocity, fixed-step RK4
  (0.01 s up, 0.2 s down), a 1200 s cap, a 4-layer atmosphere, constant gravity, and one wind vector.
...
- **Interop:** `.ork`, `.rkt` and `.CDX1` read into one model; `.ork` export.
- **Validation:** stored results in design files (35 files, 97 runs; median apogee error 2.9%, which
  is Loft's error, not a reference), a RocketPy guard fed Loft's own drag, and no real flights.
- **Worth keeping:** withholding a number with a reason instead of printing a wrong one (as
  `Option` and typed validity flags, not zeros).

## Clean-room hazards inside Loft: never port these; re-derive from format docs or jar runs

- **Code whose own comments say it came from OpenRocket's GPL Java source:**
  - CG and override precedence, `lib/sim/mass.ts:250-350`;
  - the parachute Cd 0.8, `lib/sim/recovery-defaults.ts:38-60`;
  - the centring-ring auto bore, `lib/ork/adapt.ts:1350-1393`;
  - the ground-hit frame change at OpenRocket 24.12, `lib/ork/adapt.ts:84-108`.
- **Material of unclear provenance:** the RockSim shape codes (`lib/rkt/adapt.ts:68-106`), the
  methods page's auto-radius text, OpenRocket strings quoted in Loft's `ROADMAP.md:2605-2640`, and
  a `.orc` epsilon (`BACKLOG.md:2841`).
- **Example files:** OpenRocket's example `.ork` files are GPL data. Use them as external inputs
  only, never commit them.

## Numbers not to reuse as references

- **`fixtures/rocketpy-cross-check.json`:** unpinned RocketPy, fed Loft's own Cd, mass and inertia,
  under ISA.
- **Demo "stored" figures:** hand-written, and one is physically impossible.
- **OpenRocket example results:** no version recorded.
...
|---|---|---|---|---|
| L1 | Constant gravity 9.80665 on a flat Earth; RocketPy used latitude gravity, which biased the cross-check low | lib/units.ts:52; scripts/rocketpy/fly.py:30 | M1.1 | `hpr_core::gravity::tests::somigliana_matches_published_values` |
| L2 | Geometric altitude treated as geopotential: 11 km gave 216.65 K and 22,632 Pa, where USSA76 gives 216.774 K and 22,699.9 Pa | lib/sim/atmosphere.ts:37,94 | M1.2 | `hpr_atmos::ussa76::tests::geometric_11_km_is_216_774_k_and_22_699_9_pa` |
...
| L6 | One wind vector at every altitude; no gusts | lib/sim/simulate.ts:588-596 | M1.2 | `hpr_atmos::wind::tests::layered_wind_interpolates_speed_and_heading` |
...
| L8 | Fin CNα grows linearly with fin count; no correction for 5 to 8 fins | lib/sim/aero.ts:177 | M1.5 | `hpr_aero::fins::tests::six_fin_cna_applies_fin_count_factor` |
| L9 | The conical-transition CP formula is used for every transition shape | lib/sim/aero.ts:80-86 | M1.5 | `hpr_aero::body::tests::ogive_transition_cp_uses_volume_form` |
| L10 | Elliptical-fin CNα comes from an equal-area trapezoid with the wrong mid-chord sweep | lib/sim/aero.ts:165-175 | M1.5 | `hpr_aero::fins::tests::elliptical_fin_cna_uses_zero_midchord_sweep` |
| L11 | Fin sets are merged into one for drag, so the result depends on set order (a fix was reverted 3 times) | lib/sim/aero.ts:467-495 | M1.5 | `hpr_aero::drag::tests::drag_invariant_to_fin_set_order` |
| L12 | Uncited constants: the body form factor (about 2× drag below fineness 6), friction compressibility 0.144/0.65, finish roughness table | lib/sim/aero.ts:320,600,621 | M1.5 | `hpr_aero::drag::tests::form_factor_and_roughness_match_cited_values` |
| L13 | No power-on/power-off base drag and no nozzle-exit relief | lib/sim/aero.ts:646-656 | M1.5 | `hpr_aero::drag::tests::power_on_base_drag_differs_by_nozzle_exit_area` |
| L14 | Lug drag uncited (C 0.5 on a solid disc plus a flat 0.01); rail buttons drag as lugs | lib/sim/aero.ts:723-731,789 | M1.5 | `hpr_aero::drag::tests::launch_lug_drag_matches_cited_hollow_tube_formula` |
| L15 | Bare diameter steps get no drag; shoulder drag jumps from 0.8ΔA to 0 as transition length reaches 0 | app/docs/limitations/page.tsx:472-526 | M1.5 | `hpr_aero::drag::tests::shoulder_drag_continuous_as_transition_length_tends_to_zero` |
| L16 | A silent Cd cap of 10 hides malformed geometry | lib/sim/aero.ts:768,781 | M1.5 | `hpr_aero::drag::tests::malformed_geometry_is_an_error_not_a_clamped_cd` |
...
| L21 | Fixed-step RK4 with no error control and no apogee convergence test | lib/sim/simulate.ts:436-468 | M1.6 | `hpr_sim::integrator::tests::apogee_converges_under_tolerance_halving` |
...
| L27 | Instant inflation, no opening load, an unsourced 0.5·A_ref body term, and drogue CdA still summed after the main | lib/sim/recovery.ts:41; lib/sim/simulate.ts:680 | M1.7 | `hpr_sim::recovery::tests::inflation_time_limits_peak_opening_load` |
...
| L29 | Parachute Cd default 0.8 came from OpenRocket source; RASAero documents 1.33 and RocketPy cites NASA SP-8066 | lib/sim/recovery-defaults.ts:38-60 | M1.7 | `hpr_sim::recovery::tests::default_canopy_cd_carries_its_citation` |
...
| L32 | Flutter uses 1.337 with (λ+1)/2, suspected √2 high (the unsafe side) against NACA TN 4197; unverified; 7 shear moduli unsourced | lib/sim/flutter.ts:11,287 | M1.10 | `hpr_sim::flutter::tests::flutter_constant_matches_tn_4197`, `hpr_sim::flutter::tests::scaling_laws_in_thickness_shear_modulus_and_pressure` |
| L33 | Static margin blew up as CNα went to 0 and was published as ±12 to 15 cal | COMPETITION.md:131 | M1.10 | `hpr_sim::metrics::tests::static_margin_undefined_when_cn_alpha_near_zero` |
...
| L37 | Delays split on "-" only: `P` and comma lists are lost, and the 100/1000 plugged markers become 100 s | lib/motors/eng.ts:127-132 | M1.3 | `hpr_motor::eng::tests::delay_lists_and_plugged_markers_parse` |
...
| L39 | Time samples not checked for order; burn time is the last sample, not ThrustCurve's definition | lib/motors/eng.ts:88-149 | M1.3 | `hpr_motor::eng::tests::rejects_non_monotonic_time`, `hpr_motor::tests::burn_time_matches_thrustcurve_definition` |
...
| L44 | Inertia: pitch only, rod formula without the radial term, fins ignore span, rings and masses get 0 | lib/sim/mass.ts:38-172 | M1.4 | `hpr_design::mass::tests::thin_tube_inertia_includes_radial_term` |
| L45 | Hollow transition CG uses the solid centroid; freeform fin CG hard-coded at 0.42 c_r | lib/sim/mass.ts:107,172 | M1.4 | `hpr_design::mass::tests::hollow_transition_and_freeform_fin_cg_are_exact_centroids` |
| L46 | Fin tabs never read (100 to 120 g lost on 2 designs); rail buttons came in at 0 kg | BACKLOG.md:5027; lib/ork/adapt.ts:852-869 | M1.4 | `hpr_design::mass::tests::fin_tab_and_rail_button_mass_counted` |
| L47 | Reference diameter is the widest component, which can be internal; OpenRocket also offers nose or custom | app/docs/methods/page.tsx:296-315 | M1.4 | `hpr_design::tests::reference_diameter_ignores_internal_components` |
| L48 | Tangent ogive only (no secant), a silent power-series default, Haack names swapped in a comment | lib/sim/shapes.ts:31-48 | M1.4 | `hpr_design::shapes::tests::secant_ogive_and_haack_parameters_change_profile` |
| L49 | Transitions use a "clipped nose" profile with a kink, never checked against `.ork` meaning | lib/sim/shapes.ts:61-91 | M1.4, M3.1 | `hpr_design::shapes::tests::ogive_transition_hits_both_radii_and_is_monotone` |
| L50 | A motor wider than its mount gave +69% apogee (the flattering side); fins could sit off the airframe | lib/sim/setup.ts:185-236; COMPETITION.md:129 | M1.4 | `hpr_design::checks::tests::motor_wider_than_mount_is_rejected`, `hpr_design::checks::tests::fin_root_must_touch_body` |
...
| id | lesson | Loft evidence | milestone | test to write |
|---|---|---|---|---|
| L75 | The RocketPy check wasn't like-for-like: ISA against stored conditions, different gravity, Loft's own drag and mass on both sides | scripts/rocketpy/fly.py:30-46; lib/validation/rocketpy-spec.ts:127-259 | M2.1 | `hpr_validate::rocketpy::tests::case_inputs_identical_on_both_sides` |
| L76 | "If the drift guard fails, regenerate the reference", and the reference moved with Loft's drag | scripts/rocketpy/README.md:23-29 | M2.1 | `hpr_validate::tests::reference_regeneration_requires_recorded_reason` |
...
| L78 | Suites skipped themselves without fixtures and reported green; a filter ignored 2 of 5 tool families | MAINTAINING.md:45-55; BACKLOG.md:1075-1082 | M2.1 | `hpr_validate::tests::summary_counts_run_and_skipped_cases_per_source` |
| L79 | Only 2 of 12 metrics had per-case gates; a +204% deployment velocity passed | BACKLOG.md:1787-1791 | M2.1 | `hpr_validate::tests::every_metric_has_tolerance_or_ungated_reason` |
| L80 | Same word, different quantity: optimum delay, deployment and ground-hit velocity differ by tool and OpenRocket version | lib/validation/compare.ts:105-124; lib/ork/adapt.ts:1009-1054 | M2.2 | `hpr_validate::tests::stored_metric_definitions_are_per_tool_and_version` |
...
| L82 | References 60% apart were excused as "no single target", and known issues excused the two largest misses | lib/corpus/sweep.test.ts:86-110; BACKLOG.md:1792-1799 | M2.2 | `hpr_validate::tests::disagreeing_references_are_both_reported` |
...
| L84 | Hand-written counts drifted (27 vs 35 designs); one disagreement was counted 15 times | lib/validation/stored-status.ts:15-17; ROADMAP.md:2389-2391 | M2.4 | `hpr_validate::census::tests::counts_are_generated_and_each_case_counts_once` |
...
| id | lesson | Loft evidence | milestone | test to write |
|---|---|---|---|---|
| L87 | Barrowman hand values: cone CNα 2 and CP 2L/3; conical transition 20 to 40 mm over 0.1 m gives CNα 1.5 and CP 0.05556 m; elliptical fin CP 0.288 c_r | lib/sim/aero.test.ts:69-263 | M1.5 | `hpr_aero::tests::barrowman_hand_values` |
| L88 | Drag invariants: friction non-increasing in Re and finite to M5; split fin sets drag like one set; base drag continuous at M1 | lib/sim/aero.test.ts:297-744 | M1.5 | `hpr_aero::drag::tests::drag_term_invariants` |
| L89 | Nose volumes: cone πR²L/3; tangent ogive R 0.04 m, L 0.25 m gives 6.7509e-4 m³; Haack πR²L(1/2 + 3C/16) | lib/sim/shapes.test.ts:1-58 | M1.4 | `hpr_design::shapes::tests::nose_volumes_match_closed_forms` |
| L90 | Terminal descent: 1.1 kg, Cd 0.8, 1 m canopy, 1.225 kg/m³ gives 5.294 m/s (Loft allowed ±30%) | lib/sim/simulate.test.ts:166-201 | M1.7 | `hpr_sim::recovery::tests::descent_rate_equals_terminal_velocity` |
| L91 | Staging: sustainer lights at booster burnout plus delay, mass steps at separation, an unreachable trigger never lights | lib/sim/staging.test.ts:267-311,687-800 | M1.9 | `hpr_sim::staging::tests::serial_plan_timing_and_mass_step` |
| L92 | Optimum delay is the same whether the flown delay is early or late | lib/sim/flight.test.ts:1044-1063 | M1.10 | `hpr_sim::metrics::tests::optimum_delay_independent_of_flown_delay` |
| L93 | Degenerate designs (zero radius, NaN tokens, zero fins, negative mass) must error or stay finite | lib/sim/robustness.test.ts:41-62 | M4.1 | `hpr::tests::degenerate_designs_error_or_stay_finite` |
| L94 | Monte Carlo: same seed gives identical samples; zero dispersion gives sd 0 and the nominal flight | lib/sim/montecarlo.test.ts:37-310 | M6.1 | `hpr_analysis::montecarlo::tests::zero_dispersion_reproduces_nominal_flight` |
| L95 | Ballast trim: CP 0.80 m, CG 0.55 m, 2 kg, d 0.1 m, nose 0.15 m gives 0.285714 kg for 3.0 cal; a sized canopy lands within 3% of target | lib/sim/trim.test.ts:14-72; lib/sim/recovery.test.ts:55-80 | M8.1 | `hpr_design::assist::tests::ballast_and_canopy_sizing_round_trip` |
...
| P11 | A fix reached one surface of seven; tests re-implemented the conversion they tested | HANDOFF.md:70-97; MAINTAINING.md:808-810 | Validity flags live in output types (M1.10, M4.1); code-reviewer flags tests that re-implement what they test |
...
- **Accuracy bars, as quoted:** RocketPy about 1% apogee on 3 flights; RASAero II averages 3.47%
  over 41 flights (M2.3 cites the originals).
