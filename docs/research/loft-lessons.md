...
| L12 | Uncited constants: body form factor 1+60/f³+0.0025f (1.95 at fineness 4, where Niskanen's 1+1/(2f) gives 1.13), friction compressibility 0.144/0.65, roughness values 60 µm and 2 µm that aren't in Niskanen Table 3.2 | lib/sim/aero.ts:320,600,621 | M1.5 | `hpr_aero::drag::tests::form_factor_and_roughness_match_cited_values` |
| L13 | No power-on base drag relief; Loft's comment says Niskanen has none, but its §3.4.5 subtracts the thrusting motors' area | lib/sim/aero.ts:646-656 | M1.5 | `hpr_aero::drag::tests::power_on_base_drag_subtracts_thrusting_motor_area` |
...
| L50 | A motor wider than its mount gave +69% apogee (the flattering side); fins could sit off the airframe | ROADMAP.md:8413; COMPETITION.md:131 | M1.4 | `hpr_design::checks::tests::motor_wider_than_mount_is_rejected`, `hpr_design::checks::tests::fin_root_must_touch_body` |
