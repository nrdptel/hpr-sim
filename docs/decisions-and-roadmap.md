...
| [ADR-055: M3.1c split, and the motors a `.ork` flies][adr-055] | That reading a `.ork` file's motors, recovery, stored results and pods is split into four increments; that a motor's thrust curve is the one the file carries under its digest first and hpr's bundled catalog second, matched on manufacturer and designation both; that `none` is a plugged motor and `0` a charge at burnout, from OpenRocket's technical documentation; and that the rocket flies only a configuration whose every motor has a curve and lights at launch, because hpr lights every motor at launch until staging arrives (2 of the library's 174 do) | [`.ork` design files](format/ork.md#motors-and-their-configurations) |
...
| <a id="m3-1c2"></a>[M3.1c2][phase-1] | When each parachute and streamer opens, and when each stage separates | not yet done |
