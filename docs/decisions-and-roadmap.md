...
| [ADR-052: What a `.ork` value means][adr-052] | That an automatic dimension keeps both its flag and the number OpenRocket last worked out, rather than becoming a hand-typed one; that either name of a renamed tag may be read, because where OpenRocket writes both it agrees with itself on all 777 elements of the corpus; that a stated zero is a value, which is what an override to no drag at all needs; and that the single flag the three subcomponent-override flags replaced is read as setting all three, out loud, since no file carries both forms | [`.ork` design files](format/ork.md#the-values-inside-the-tags) |
