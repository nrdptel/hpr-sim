...
| [ADR-073: Each named cause sized by OpenRocket's own flight without it][adr-073] | [M2.2e4](#m2-2e4) sizes the two causes named for the five apogees more than 5% from OpenRocket's: OpenRocket flies each flight again with nothing deployed and, where a part is told it has no drag, with that setting cleared, which is what hpr reads. Against those flights all five are within 5%, from −1.07% to +4.79%. What is left on one design has a lead, not an explanation ([#177](https://github.com/nrdptel/hpr-sim/issues/177), a very blunt nose's drag). | [`.ork`: the two named causes, and their size](format/ork.md#hprs-flights-against-openrockets) |
...
| <a id="m2-2e4"></a>[M2.2e4][phase-1] | A written cause for every apogee more than 5% from OpenRocket's, sized by OpenRocket's own flight without it: all five within 5% after, −1.07% to +4.79% ([ADR-073][adr-073], [results](format/ork.md#hprs-flights-against-openrockets)) | done |
