...
| [ADR-065: Stored results are references only when current and structurally plausible][adr-065] | Stored results remain readable, but only `uptodate` runs with complete finite and internally consistent ascent summaries are reference-eligible; stale statuses and contradictions are counted with stable reasons and never enter gates or denominators | [`.ork` stored simulations](format/ork.md#what-openrocket-last-did-stored-simulations) |
