...
| [ADR-067: Curves come from OpenRocket's own database by digest, each held to its impulse][adr-067] | Most designs name their motor's curve by OpenRocket's digest and do not carry it. An oracle records the motor database OpenRocket 24.12 ships, and `cargo xtask ork` supplies each solid curve for its digest, never by name. Every curve it builds (1,288) and every curve the library embeds (3) is bit for bit OpenRocket's total impulse, held to 0.1%. Of the 162 configurations the bundled catalog left without a curve, 66 now fly, 72 are held back for another named reason and 24 still have none, each with its reason. The database and the designs' curves stay private, counted only. | [The `.ork` format: motors](format/ork.md#motors-in-the-reference-library) |
