...
| [ADR-060: M2.2 split, and the structure's mass held to OpenRocket's][adr-060] | That [the OpenRocket oracle milestone](#m2-2) goes mass first, then OpenRocket's mass conventions, its motors, public flights and the corpus; that each design's structure is held to OpenRocket's in mass (1%) and centre of mass (1% of length), thresholds set before measuring, with every design outside given its cause; and that which of OpenRocket's inertias is roll is measured on a tube worked by hand | [Mass properties](physics/mass.md#checked-against-openrocket) |
